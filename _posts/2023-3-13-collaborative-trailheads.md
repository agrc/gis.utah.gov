...
At UGRC, we see OpenStreetMap (OSM) as an ideal platform for managing and maintaining datasets such as trails, trailheads, campgrounds, ski lifts, cross-country ski trails, common places, and a variety of other layers - because who better to steward this data than all of us? 
...
To make this process fun (and easy), we created a simple map challenge and opened it up to the mapping community at large. So, if you’ve ever ventured out in Utah, it’s likely you’ll have something to contribute.
...
As we move forward with this model, we will also be working with the authoritative stewards to see how this project fits into similar initiatives that are already underway such as the OSM for Government and the Public Domain Map.
...
- How to create an OSM account and link it to the map challenge
- How to add a new trailhead to OSM (scenario one of the map challenge)
- How to update an existing OSM trailhead (scenario two of the map challenge)

### Follow Along with the Project
{: .flex .flex--left}

You can follow along with this project by visiting our collaborative trailheads GitHub repository or by reaching out to us directly.
