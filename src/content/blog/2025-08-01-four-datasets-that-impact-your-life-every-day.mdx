...
[Address points](/products/sgid/location/address-points/) are exactly what they sound like--it's a points dataset where each and every data point represents a unique address in the United States Postal Service system. Throw all those points on a map, and they look something like this:

(will insert map early next week)
...
Many other public and private entities also use address points for a process known as geocoding. In fact, UGRC has our own in-house [geocoding API](/products/api/) that uses the latest addressing data to ensure the most accurate results. You can learn more and start geocoding your own addresses by [signing up for the API client](https://developer.mapserv.utah.gov/) on our website.
...
The address you entered into your phone for a pizza order or a package delivery isn't just a random series of letters and numbers. It's a unique location on an addressing system. We can make digital representations of these addressing systems to assign addresses to certain locations--and to keep them from getting assigned to the wrong area. With the address system in place, we're ready to pinpoint where each individual address lies on a map.
...
While we've picked out just a few datasets to chat about, there's many more that impact daily life. There's the [Public Safety Answering Point](/products/sgid/society/public-safety-answering-point-boundaries/) (PSAP) boundaries, which are used to make sure your 911 call goes to the correct dispatch center. There's all our [parcel](/products/sgid/cadastre/parcels/) and [land ownership data](/products/sgid/cadastre/land-ownership/), which are used for all kinds of analytics and operations planning. And beyond these, there's so many other 'little brown data birds' that elevate the quality of life for residents in Utah. These datasets, along with more than 300 other unique layers, can be found in the [Statewide Geographic Information Datasource](/products/sgid/), or SGID. You can browse these datasets using our [SGID Index](/products/sgid/sgid-index/) or by viewing our array of [data categories](/products/sgid/categories/). For questions or comments regarding SGID data, don't hesitate to reach out to [our team](/contact/).
