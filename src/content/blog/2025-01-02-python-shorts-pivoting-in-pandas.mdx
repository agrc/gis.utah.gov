...
Today we'll use the [Housing Unit Inventory](https://gis.utah.gov/products/sgid/planning/housing-unit-inventory/) (HUI) and Pandas' `unstack`, `pivot`, and `pivot_table` to investigate how the type of housing units built (single-family, townhome, apartment, etc) has changed over time.

## Data Has Body Types? Wide Versus Long

Before we can dive into these tools, we need to cover some background theory first on data shape. Neatly-organized data tables can fall on either end of a spectrum from ["wide" to "long"](https://towardsdatascience.com/long-and-wide-formats-in-data-explained-e48d7c9a06cb).
...
<Image src={UnstackedPlot} loading="eager" alt="Too many axis labels." />
...
Because our series has a hierarchical MultiIndex, we can use Pandas' ['unstack'](https://pandas.pydata.org/docs/user_guide/reshaping.html#stack-and-unstack) to widen our data to something a little more useful.
...
<Image src={StackedDataframe} loading="eager" alt="A pretty two-dimensional dataframe table." />
...
<Image
  ...
  loading="eager"
  alt="It's got the same values as the unstacked dataframe but a different column order."
/>
...
<Image src={PivotTable} loading="eager" alt="Are you tired of this table yet?" />

So why didn't we just lead with this? First, this is a more limited and specific case of pivoting for when you want to perform one of the common aggregator functions (Which functions are those? As usual, [StackOverflow to the rescue](https://stackoverflow.com/questions/53943319/what-are-all-pandas-agg-functions)! Basically, it'll try anything you throw at it, but there are some common ones.).
...
But more importantly to me, understanding what `unstack` and `pivot` were doing helped me to really grasp how `pivot_tables` works. My data exploration often begins with the thought process "what happens if I group the data by this column?" and flows from there. If you're always working with the same type of data, maybe you don't have to go through that exploration phase and can go straight to `pivot_table`. But that's one of the perks of working at UGRC: we're often looking at very different datasets from one project to the next.

I hope this exploration into the theory of data table shapes and examples of some of the reshaping tools available helps you better understand what tools to use when your data need a little love.
