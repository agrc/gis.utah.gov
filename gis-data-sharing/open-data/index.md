...
If your department or division has uploaded your datasets to your own ArcGIS Online organization, you can share your datasets through the [SGID Open Data](https://opendata.gis.utah.gov/) website. This is a great way to make them easily discoverable by anyone searching for Utah GIS data. Because they are your ArcGIS Online items, you have full control over when they are updated.

## Adding Your Data to Open Data

All you need to do is **share your ArcGIS Online item with the appropriate SGID group**. These are ArcGIS Online groups that have been enabled for Open Data—items shared with these groups are automatically added to the SGID Open Data site.

First, you have to be a member of the SGID groups your datasets best fit in. Navigate to AGRC's [groups page](https://utah.maps.arcgis.com/home/groups.html) and click on the appropriate group. Then click on "Join Group" to request access. AGRC will verify your request and add you to the group.

Once you're a member of the right groups, share each item to the appropriate group. You also need to make sure the item is shared with "Everyone (public)" as well. Otherwise, it will only be visible to you (or other members of your organization if shared to the organization) in Open Data.
...
- A descriptive, department/division-based **user name** helps users know where the dataset is coming from. If possible, we prefer the item be owned by a user whose name includes the department/division's name or function. For example, all the datasets we share are owned by the `UtahAGRC` user.
- Open Data displays the text from the `Description` field of the source ArcGIS Online item for it's **description**. You shouldn't count on any information in the ArcGIS Online `Summary` field to show up unless it's repeated in the description.
...
- By default, users can't download spatial datasets from ArcGIS Online/Open Data as **File Geodatabases**. To enable this, go to the item's page in ArcGIS Online and click on the "Settings" tab. Scroll to the bottom and check the box to "Allow others to export data to different formats".
  - NOTE: This will also allow people to export some or all of the features if they add the dataset as a layer in ArcGIS Pro.

## Taming the Sources List

When a user clicks on a data category or searches for a dataset on the SGID Open Data site, a "sources" filter list appears on the left. This allows users to filter results to a specific "source". Currently, **it can only show a max of ten sources**. Once we have more than ten in all our Open Data (not just the current search), we don't have any control over which ones are shown. Unfortunately, the free-entry text box above the list requires a perfect text match, so it doesn't help users discover other sources either.

We've tried to be careful with our data to keep any superfluous sources out of this list, and we'd appreciate your help with this as well.

So where do the sources come from? Esri explains it [in this FAQ](https://doc.arcgis.com/en/hub/get-started/frequently-asked-questions.htm#GUID-9843B713-46D2-4938-A961-EC0CD81AE410). The main take away is that it first searches the **ArcGIS Online item's metadata** for an organization name. If it doesn't find anything there, it falls back to your **ArcGIS organization name**.

If you want to have full control over the name displayed in the sources list, we suggest setting it in the `Resource > Citation > Contact > Organization Name` metadata field. Otherwise, we suggest you make sure the metadata contact fields are empty and let it pick up your ArcGIS Online organization name. This second way is where the "Utah Automated Geographic Reference Center (AGRC)" text comes from on all our datasets.
